...
In my automation framework, I have used inheritance mainly between the Base Class and Page Classes.

The Base Class contains common setup like WebDriver initialization, browser launch, and teardown methods.
All my Page Classes extend this Base Class, so they can directly reuse the driver instance and common methods without redefining them.

This helps in reducing code duplication and keeps the framework maintainable.
...
In short, `super` helps the child class explicitly use the parent’s version.
...
- If we don’t write `super()` explicitly, Java inserts it automatically **only if the parent has a default constructor**.
...
- and we want to clearly refer to the parent’s variable
...
This is the **main reason `super` exists** — to allow access to parent behavior even after overriding.

---

## Q8. What happens if you don’t write `super()` explicitly in child constructor?
...
`super` is used to access parent class constructor, methods, or variables — especially when the child overrides behavior but still needs parent logic.
...
```
---
Which methods are called at runtime — parent or child?
...
## Q2. Which members are decided at compile time and which at runtime?
	•	Methods → runtime (dynamic binding)
	•	Variables → compile time (based on reference type)
---
...
→ Object created is Child
System.out.println(p.x);
→ x is a variable
→ Variables are decided at compile time
→ Compiler checks reference type (Parent)
→ Prints Parent.x → 10
	•	p.show();
→ show() is a method
→ Methods are decided at runtime
→ JVM checks actual object (Child)
→ Calls Child.show() → “Child show()”***
```
...
# Framework Design Question: Why Use WebDriver Reference?
...
- Framework code depends on the **parent type (WebDriver interface)**, not a concrete child implementation.
- Makes it easy to:
...
We use the `WebDriver` reference so we can switch browsers (Chrome, Firefox, Edge) without changing test code.
...
We try to make our framework dependent on the interface rather than the concrete class because it provides loose coupling, and later we can change the implementation without modifying the code.
