...
It can contain abstract methods, default methods, static methods, and private methods (Java 9+).
...
## 1. Abstract Methods (Core of Interface)
...
You don’t write `abstract`, but it is always there by default.
...
✔ Introduced to avoid breaking existing code
...
Default methods give **flexibility** + **backward compatibility**.
...
✔ Have implementation  
✔ Belong to interface (not object)  
...
Static methods are **class-level**, so there is **no polymorphism**.
...
An interface contains abstract methods by default, and from Java 8 onwards it can also have default and static methods with implementation. Abstract methods must be implemented, default methods are optional to override, and static methods belong to the interface and cannot be overridden.
